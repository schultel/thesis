...
\begin{description}
 ...
 \item[Purity:] The goal of all experiments is to reconstruct the neutrino
  events as accurately as possible. Therefore it is desirable to record a
  large number of unscattered photons, for which a very clear environment is
  needed. There might
  be, however, situations where scattering is eligible, e.\,g.\ when only the
 ...
 \item[Shielding:] In high-energy neutrino experiments, muons from atmospheric
  ...
  placed deep underground so that there is a shielding of several hundred
  meters thickness, comparable to the penetration depth of $\mathcal{O}
...
absorption.}.
Above it, ten DOMs with 10\,m spacing are attached to each DeepCore string to
serve as a veto against atmospheric muons \cite{I3Design,DCDesign}.
...
Figs.~\ref{fig:string_layout} and \ref{fig:string_layout_side}, the string 
spacing is 22\,m while the PDOMs are located at the lowest 300\,m of IceCube 
with a vertical distance of 3\,m. In this depth, the same as the main part of 
DeepCore, the ice has the best optical properties.
...
As shown in Sec.~\ref{sec:PINGUosc}, the neutrino mass hierarchy
...
% PINGU that are imprinted by the neutrino mass hierarchy,
it is crucial to
...
coincidence (HLC) within a time window of 2.5\,\textmu s. A HLC
means that two DOMs on the same string that are not more than two vertical
positions (\ie 6\,m) apart have launched within 1\,\textmu s. For the PINGU
trigger, all PINGU DOMs plus the DeepCore DOMs and the ones one the neighbouring
IceCube strings are taken into account.

If an event satisfies the PINGU trigger, the full detector including
...
PINGU, two different cleanings are applied consecutively: first a seeded
...
leads to smoother results and significantly reduces the amount of data that
...
The Monopod algorithm is a likelihood reconstruction for cascade-like events
...
fixed at the cascade vertex, the muon adds only three variables to the problem:
...
Hence, only the muon energy $E_\mu$, which is proportional to its length as 
...
Since in PINGU the expected rate of neutrino events is on the order of $1 -
...
previous DeepCore analyses, in which atmospheric neutrino oscillations have 
already been measured at higher energy \cite{DCosc}.
...
\begin{figure}
 \centering
 \includegraphics[width=0.7\textwidth]{selection_eff}
...
The probability of passing all these cuts apart from the final directional cut
...
\begin{eqnarray}
 ...
 \Rightarrow E_\nu &=& \frac{(m_\tau + m_p)^2 - m_p^2}{2 m_p} =
  3.46\,\mathrm{GeV}
\end{eqnarray}
Neutral current events of all flavours finally show a behaviour similar to \nue 
and \nuebar CC events as they correspond to point-like emissions
...
the underlying event \cite{PID_BDT}. Three of these variables are based on the
distribution of the reduced arrival times $t_i^\mathrm{red}$ of the DOM hits
($\vec{r}_i,\,t_i$) in the event with respect to the reconstructed vertex
($\vec{x},\,t$),
\begin{equation}
 t_i^\mathrm{red} = t_i - t - |\vec{r}_i - \vec{x}|/c \quad,
\end{equation}
while the remaining three refer to the result of the MultiNest reconstruction:
\begin{description}
 ...
 \item[Muon fractional energy:] The fraction of the total reconstructed energy
  ...
  to the above, this is a measure of how prominent the muon features are in the
...
\centering
  \subfloat[\label{fig:mDOM}]
    {\includegraphics[height=6cm]{mDOM_cropped}}\qquad
  \subfloat[\label{fig:WOM}]
    {\includegraphics[height=6cm]{WOM}}
...
% TODO: re-do plots, adjust numbers
...
ensures that a large fraction %(up to 75%)
...
possible, in this case a large difference in refractive index is needed to
...
% EJ-298 wavelength-shifting paint which consists of a fluorescent dopant and a 
% PVT base \cite{bib:paintprops}. A film  thickness of O(10\,$\mu$m)) is 
% sufficient to effectively absorb all incoming photons. 
...
\item[Photon capture efficiency]
 \begin{figure}[t]
  \centering
   \includegraphics[width=0.7\textwidth]{capture_eff_tube_with_spectrum}
...
This quantity has been measured in a lab setup using a monochromator with 
...
\begin{figure}[htb]
\centering
  \subfloat[\label{fig:angular_eff}]
    {\includegraphics[width=0.495\textwidth]{ang_eff_both}}
  \subfloat[\label{fig:full_eff}]
    {\includegraphics[width=0.495\textwidth]{all_modules_QE}}
  \caption{Relative angular acceptance \protect\subref{fig:angular_eff} and full
    module photo-detection efficiency at optimal illumination angle
    \protect\subref{fig:full_eff} of WOM and IceCube DOM.}
...
% \begin{figure}[tbh]
%   \centering
%   \includegraphics[width=0.7\textwidth]{all_modules_QE}
% %   \includegraphics[width=0.5\textwidth]{icrc2013-0857-04}
%   \caption{Full module photo-detection efficiency of WOM and IceCube DOM at 
% optimal illumination angles.}
%   \label{fig:full_eff}
% \end{figure}

To obtain the detection efficiency of the fully assembled module, the three 
...
\begin{equation}
 \overline{ME} = %\bar\varepsilon_\Omega \cdot 
...
Assuming a WOM with a diameter of 20\,cm and a length of 2\,m as above and, for 
comparison, a DOM with 30\,cm diameter \cite{ICdom}, one can calculate the
...
Hence in terms of total effective area, 
\begin{equation}
...
% If one would reduce the considered wavelength range to 300\,--\,600\,nm, the
% mean module efficiency increases by about 40\,\% for the DOM (since its
% ``blind'' regime below 300\,nm does not enter in the calculation), while for the
% WOM it remains essentially constant.

...
\begin{description}
 \item[$\mathbf{z_\mathrm{vertex} < -200}$\,m:] The very first cut requires the
  ...
  which is 20\,m below the topmost PINGU DOMs. Since atmospheric muons enter
...
background  muons deposit their energy evenly on their rather long path through
the  detector, while neutrinos are almost point-like sources in both location
and  time.

\item[$\mathbf{r_\mathrm{vertex} < 95}$\,m:] Looking at the secondary ``Timed

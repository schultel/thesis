...
in detail. Here the focus will be on neutrinos and their properties and
interactions, while neutrino oscillations will be described in the
following chapter.
...
\caption{The fundamental particles in the Standard Model. Figure taken from 
  \cite{SMchart}.}
\label{fig:SMchart}
\end{figure}
...
them---if any---can have zero mass, while the others must correspond to finite
mass.
...
As mentioned above, neutrinos do not participate in the strong and
...
the then undetectable neutrino. Since only the daughter nucleus and the
...
be mono-energetic. Observations showed, however, a broad spectrum in energy
instead of a single line. Without violating the conservation of energy, this
can only be achieved if a third particle is produced in the process, which has
to be electrically neutral but can carry away energy and momentum. 
...
shown in Fig.~\ref{fig:beta_minus}. The $W$ boson subsequently decays into an
electron and a \nuebar or positron and \nue, respectively\footnote{In
principle, a $W$ boson can decay into any charged lepton and corresponding
neutrino. Yet in natural radioactivity the typical decay energies are few MeV,
hence the $W$ boson is only virtual and since the overall energy has to be
conserved, muons or tau leptons are too massive to be produced.}.

\begin{figure}
 \centering
 \begin{fmffile}{beta_minus}
 \begin{fmfgraph*}(70,40) \fmfpen{thin}
 \fmfstraight
...
Since in each of those $\beta^-$ decays a neutrino is emitted, a nuclear reactor
provides a strong and steady flux of \nuebar in the low MeV range, which can be
monitored via the
...
\begin{eqnarray}
 \pi^\pm &\to\ & \mu^\pm + \overset{(-)}{\numu}  \label{eqn:pi_decay} \\
 K^\pm   &\to\ & \mu^\pm + \overset{(-)}{\numu}  \label{eqn:K_decay}\\
\end{eqnarray}
...
are monoenergetic at 1.44\,MeV. Once a deuteron is produced, it quickly
($\approx 1$\,s) merges with another proton to $^3_2\mathrm{He}$ and emits a
photon.
...
energy. During the past decades, the spectrum of these particles, the
...
energies \cite{CosmRaySpecHillas}.
...
develops, which then travels down to Earth.

Main components of these particle showers are kaons, pions, and muons. All of 
these particles
are relatively short-lived\footnote{$\tau_\mu = 2.2 \times 10^{-6}$\,s,
$\tau_K = 1.2 \times 10^{-8}$\,s, and $\tau_\pi = 2.6 \times 10^{-8}$\,s
\cite{PDG}} and produce neutrinos in their decay, according to 
(\ref{eqn:pi_decay}), (\ref{eqn:K_decay}), and
\begin{equation}
 \mu^\pm \to e^\pm + \overset{(-)}{\nue} + \overset{(-)}{\numu}
\end{equation}
...
shown in Fig.~\ref{fig:AtmFlux}, these predictions show good
...
Neutrinos scattering off a free electron will serve as an introductory example.
The cross-section is \cite{NuXsec_review}
\begin{equation}
...
($W^\pm$ exchange, CC) 
\begin{equation}
 \mathcal{M}_\mathrm{CC} = -\frac{G_F}{\sqrt{2}}
 \left\{\left[\bar l \gamma^\mu\left(1-\gamma^5\right)\nu_l\right]
       \left[\nuebar\gamma_\mu\left(1-\gamma^5\right)e\right]\right\}
\end{equation}
and a neutral current ($Z^0$ exchange, NC) contribution,
\begin{equation}
 \mathcal{M}_\mathrm{NC} = -\sqrt{2}G_F
\left\{\left[\bar\nu_l\gamma^\mu\left(g^\nu_V-g^\nu_A\gamma^5\right)\nu_l\right]
       \left[\bar e\gamma_\mu\left(g^f_V-g^f_A\gamma^5\right)e\right]\right\}
       \quad,
\end{equation}
as shown in Fig.~\ref{fig:nue_e_CC}. The vector and axial couplings of the 
involved leptons are given by:
\begin{eqnarray}
 g^\nu_V &=& g^\nu_A = +\frac{1}{2} \\
 g^f_V   &=& -\frac{1}{2} + 2\sin^2\thet{W} \\
 g^f_A   &=& -\frac{1}{2}
\end{eqnarray}
...
\begin{equation}
 \frac{d\sigma}{dy}\bigg\rvert_\mathrm{CC} = \frac{2m_e G_F^2 E_\nu}{\pi}
...
$10^7$\,eV, well above the electron mass. The sharp peak at $\approx 
10^{16}$\,eV
...
understood and have to be modelled in Monte Carlo based simulations. However,
...
\begin{description}
 \item[Electron:] In dense matter, a GeV electron will induce an
  ...
  below one metre, as water has a density of $\varrho_\mathrm{H_2O} =
  1$\,g/cm$^3$.
...
\begin{description}
 ...
 \item[Cascades] encompass all other types of events. A hadronic shower is 
  ...
  shower (if it exists) is much smaller than a millimetre and hence not 
  ...
  The achievable directional resolution is worse than for tracks since 
...
Usually waves from neighbouring atoms cancel, such that no net effect is 
observable. 
...
the constants, one finds for particles of unit charge ($Z^2 = 1$) in ice:
\begin{equation}
  \frac{d^2 N_{\gamma}}{dx\,dE} 
...
certain particle. Again using the values for ice, the calculation yields a 
number of
\begin{equation}

...
\begin{equation}
 P_{\alpha\to\beta} = \left|\Braket{\nu_\beta
                             \left|\,\mathcal{U}_\mathrm{PMNS}^\dagger\,
                             \left|\,\hat{H}\,\right|\,
                             \mathcal{U}_\mathrm{PMNS}\,\right|
...
\end{equation}
In the mass base, the Hamiltonian can be replaced by an effective one containing
the squared masses:
\begin{equation}
 \hat{H}^\mathrm{eff}
...
The mixing angles define the amplitude of the oscillation, with $\theta
= \ang{45}$ giving rise to so-called ``maximum mixing'' where a full
...
to be experimentally challenging.
...
neutrinos that can be associated with a transient optical event such as a
...
\end{equation}
Yet since this potential affects all flavours and hence all mass eigenstates in
...
\end{equation}
Here and in the following, the index has been dropped from
$\mathcal{U}_\mathrm{PMNS}$ to improve readability.
...
neutrino flavours here, without loss of generality. In this case,
...
(by being replaced by $\bar{A}_\mathrm{CC}=-A_\mathrm{CC}$ for antineutrinos)
...
done by the SNO experiment, which measured both the flux of solar \nue and the
...
potential of the Earth is characterised by two major regions: the core in the 
...
un-oscillated neutrino flux and the detector acceptance, and one identical
``far'' detector in the first oscillation minimum, usually at several kilometres
distance.
...
on \thet{13} from Daya Bay and RENO that were published shortly before. There
...
\begin{eqnarray}
 ...
 \dm{} &\equiv& m_3^2 - \left(m_2^2 + m_1^2\right)/\,2
   = \left(\dm{31}+\dm{32}\right)/\,2 = \dm{31} - \dm{21}/\,2 \enspace
   \begin{cases}
    \enspace > 0 &\mbox{for NH} \\ \enspace < 0 &\mbox{for IH}
   \end{cases}
\end{eqnarray}
...
al.~\cite{Fogli}, used throughout this thesis.}
...
arrive---this determines the distance travelled since their production in
...
$\bar\nu_\alpha \to \bar\nu_\beta$. Differences arise from the MSW resonance,
easy to spot in the energy range from 2 to \SI{10}{\GeV} for zenith angles
between $\coszen \approx -0.9$ and $-0.4$, that appears in neutrinos for the
normal and in antineutrinos for the inverted mass hierarchy
(cf.~Sec.~\ref{sec:matter_osc}).
...
\enlargethispage{\baselineskip}
Yet since the fluxes of neutrinos and antineutrinos of the same flavour are
...
the hierarchy is normal. In this case roughly \sfrac{2}{3} of the
events---the neutrino-induced ones---are affected by the resonance, while in the
inverted hierarchy case only the remaining third caused by antineutrinos is.
